...
class RemoveGravityformsLicenseNotice {
	...
	public function plugins_loaded() {
		...
		if ( is_admin() && class_exists( 'GFForms') && null !== RG_CURRENT_PAGE && RG_CURRENT_PAGE == "plugins.php" ) {
			...
			add_action( 'init', array( 'GFAutoUpgrade', 'cleanup_gf_addons_notices' ) );

			add_action( 'wp_loaded', 'cleanup_gravityforms_notices', 20 );

   function cleanup_gf_addons_notices() {

    remove_action( 'after_plugin_row_gravityformsaweber/aweber.php', array( 'GFAutoUpgrade', 'rg_plugin_row' ) );
